...
const WalletModal = ({ isOpen, onClose, onSelect, connectedWallet = "" }) => {
  if (!isOpen) return null;

  const wallets = [
    { name: "MetaMask", icon: "MM", status: "Detected" },
    { name: "Phantom", icon: "PH", status: "Detected" },
    { name: "WalletConnect", icon: "WC", status: "" },
    { name: "Coinbase Wallet", icon: "CB", status: "" },
  ];
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[380px] bg-[#0B0E14] border border-white/10 rounded-[2rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-white/5 flex justify-between items-center">
          <h3 className="font-bold uppercase tracking-[0.1em] text-white">
            Connect Wallet
          </h3>
          <button
            onClick={onClose}
          ...
          {wallets.map((wallet) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className="h-8 w-8 rounded-full bg-white/10 text-white text-[10px] font-black flex items-center justify-center tracking-widest">
                  {wallet.icon}
                </span>
              ...
              {connectedWallet === wallet.name ? (
                <span className="text-[10px] bg-teal-500/10 text-teal-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-tighter">
                  Connected
                </span>
              ) : wallet.status ? (
                ...
                <span className="text-[10px] bg-white/10 text-gray-300 px-2 py-0.5 rounded-full font-bold uppercase tracking-tighter">
                  Connect
                </span>
              )}
            </button>
