...
import { Gem, Zap, Calendar, Trophy, AlertTriangle, Pickaxe } from "lucide-react";
...
const UserMining = () => {
  ...
  // --- PERSISTENT STATE ---
  const [diamonds, setDiamonds] = useState(() => Number(localStorage.getItem("mining_diamonds")) || 0);
  const [streak, setStreak] = useState(() => Number(localStorage.getItem("mining_streak")) || 0);
  const [lastMined, setLastMined] = useState(() => localStorage.getItem("mining_last_date") || "");
    ...
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto bg-black p-4 md:p-8 flex flex-col items-center scrollbar-hidden">
        ...
        <header className="w-full max-w-4xl mb-8 flex justify-between items-end border-b border-white/5 pb-6">
          <div>
            <h2 className="text-3xl font-black uppercase italic text-orange-500 tracking-tighter">Diamond Extraction</h2>
            <p className="text-[10px] text-gray-500 uppercase tracking-[0.3em] font-bold">Node: Active // Secure Connection</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full max-w-4xl">
          ...
          <div className="lg:col-span-2 bg-[#0B0E14] border border-white/10 rounded-[2.5rem] p-8 flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className={`relative z-10 p-12 rounded-full border-4 border-dashed transition-colors duration-500 ${canMineToday ? 'border-orange-500 animate-spin-slow' : 'border-gray-800'}`}>
               <div className={`w-32 h-32 rounded-full flex items-center justify-center bg-gradient-to-br from-orange-500 to-orange-700 shadow-[0_0_50px_rgba(249,115,22,0.3)] ${isMining ? 'animate-bounce' : ''}`}>
            ...
            >
              {isMining ? "Authorizing..." : canMineToday ? "Initiate Extraction" : "Extraction Locked"}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0B0E14] border border-white/10 rounded-[2rem] p-6 relative overflow-hidden">
              <div className="flex justify-between items-start mb-4">
                <p className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Mining Streak</p>
                <Calendar size={14} className="text-orange-500" />
              </div>
              <div className="flex items-end gap-2">
                <span className="text-5xl font-black italic text-white">{streak}</span>
                <span className="text-sm font-bold text-orange-500 uppercase mb-2">Days</span>
              </div>
            ...
            <div className="bg-orange-500/5 border border-orange-500/10 rounded-[2rem] p-6">
              <div className="flex items-center gap-3 mb-4 text-orange-500">
                <Trophy size={20} />
                <h4 className="text-xs font-black uppercase italic tracking-widest">Protocol Rewards</h4>
              </div>
